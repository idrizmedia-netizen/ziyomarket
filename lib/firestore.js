...
export async function addCategory(name, createdBy) {
...
export async function updateCategory(categoryId, name) {
  // shu bo'limdagi barcha mahsulotlarning categoryName maydonini ham yangilaymiz
  const q = query(collection(db, "products"), where("categoryId", "==", categoryId));
  const snap = await getDocs(q);
  const batch = writeBatch(db);
  snap.docs.forEach((d) => batch.update(d.ref, { categoryName: name }));
  batch.update(doc(db, "categories", categoryId), { name });
  await batch.commit();
}
...
export async function addProduct(categoryId, categoryName, data, createdBy) {
...
export async function placeOrder({ uid, buyerName, buyerEmail, buyerPhone, items, pickupTime, note }) {
  ...
  await addDoc(collection(db, "orders"), {
    ...
    total,
    note: note || "",
    pickupTime: pickupTime || null, // "YYYY-MM-DDTHH:mm" (mahalliy vaqt)
    status: "pending",
    createdAt: serverTimestamp(),
  });
}

// Muddati o'tgan (olib ketilmagan) buyurtmalarni avtomatik bekor qiladi.
// Bu funksiya har safar buyurtmalar ro'yxati o'qilganda (xaridor yoki
// sotuvchi tomonidan) chaqiriladi — alohida serverga ehtiyoj qoldirmaydi.
export async function autoCancelExpiredOrders(orders) {
  const now = Date.now();
  const expired = orders.filter(
    (o) => o.status === "pending" && o.pickupTime && new Date(o.pickupTime).getTime() < now
  );
  await Promise.all(
    expired.map((o) =>
      updateDoc(doc(db, "orders", o.id), {
        ...
        cancelReason: "Belgilangan vaqtda olib ketilmadi",
      }).catch(() => {})
    )
  );
}

export async function fulfillOrder(orderId, sellerEmail, sellerName) {
  await runTransaction(db, async (tx) => {
    ...
    productSnaps.forEach((snap, idx) => {
      tx.update(productRefs[idx], {
        qty: snap.data().qty - order.items[idx].qty,
        soldCount: (snap.data().soldCount || 0) + order.items[idx].qty,
      });
    });
    ...
    tx.update(orderRef, {
      ...
      fulfilledByName: sellerName || sellerEmail || null,
    });
  });
}
...
export async function createDirectSale({ sellerEmail, sellerName, items, discount = 0, bonus = 0 }) {
  await runTransaction(db, async (tx) => {
    ...
    productSnaps.forEach((snap, idx) => {
      tx.update(productRefs[idx], {
        qty: snap.data().qty - items[idx].qty,
        soldCount: (snap.data().soldCount || 0) + items[idx].qty,
      });
    });
...
export async function updateOrderItems(orderId, items) {
  const total = items.reduce((s, it) => s + it.price * it.qty, 0);
  await updateDoc(doc(db, "orders", orderId), { items, total });
}
...
export async function addAd({ image, description, link, startDate, endDate }) {
...
// Foydalanuvchining barcha mahsulotlarga yozgan sharhlari (profilida ko'rsatish uchun)
export function subscribeMyReviews(uid, callback) {
  const q = query(collection(db, "reviews"), where("buyerUid", "==", uid));
  return onSnapshot(q, (snap) => {
...
export async function addReviewReply(reviewId, replyText, sellerEmail) {
  await updateDoc(doc(db, "reviews", reviewId), {
    sellerReply: replyText,
    sellerReplyAt: serverTimestamp(),
    sellerReplyBy: sellerEmail || "",
  });
}

// ---------- Sotuvchi-admin chat ----------

export function subscribeSellerChat(sellerEmail, callback) {
  const q = query(
    collection(db, "sellerChats", sellerEmail, "messages"),
    orderBy("createdAt", "asc")
  );
  return onSnapshot(q, (snap) => {
    callback(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
...
export async function sendSellerMessage(sellerEmail, { senderEmail, senderName, text, fromAdmin }) {
  await addDoc(collection(db, "sellerChats", sellerEmail, "messages"), {
    senderEmail,
    senderName,
    text,
    fromAdmin: !!fromAdmin,
...
// ---------- Sotuvchi arizasi ----------
// Hujjat ID'si = foydalanuvchi UID (bir foydalanuvchi — bitta ariza,
// qayta topshirilsa ustidan yoziladi).

export async function submitSellerApplication({ uid, name, email, phone, storeName, address, message }) {
...
export function subscribeMySellerApplication(uid, callback) {
  return onSnapshot(doc(db, "sellerApplications", uid), (snap) => {
    callback(snap.exists() ? snap.data() : null);
  });
}

export function subscribeSellerApplications(callback) {
  return onSnapshot(collection(db, "sellerApplications"), (snap) => {
    callback(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
  });
}

export async function approveSellerApplication(app, adminEmail) {
  await addSeller(app.email, app.name, adminEmail, {
    phone: app.phone,
    storeName: app.storeName,
    address: app.address,
    sellerType: "vendor",
  });
  await updateDoc(doc(db, "sellerApplications", app.uid), {
    status: "approved",
...
export async function rejectSellerApplication(uid, adminEmail) {
...
// Sotuvchi hujjatining to'liq ma'lumotini qaytaradi (sellerType va h.k. bilan)
export async function getSellerDoc(email) {
...
export async function addSeller(email, name, addedByEmail, extra = {}) {
  await setDoc(doc(db, "sellers", email.trim()), {
    name: name || "",
    phone: extra.phone || "",
    storeName: extra.storeName || "",
    address: extra.address || "",
    sellerType: extra.sellerType || "staff", // "staff" = faqat admin qo'shgan tovarni sotadi, "vendor" = o'z tovarini qo'shadi va sotadi
    addedBy: addedByEmail || "",
    addedAt: serverTimestamp(),
  });
}
...
// ---------- Sotuvchi vitrinasi (ochiq sahifa) ----------

export async function getSellerInfo(email) {
  const snap = await getDoc(doc(db, "sellers", email));
  if (snap.exists()) return { email, ...snap.data() };
  return { email, name: email };
}

export function subscribeSellerProducts(email, callback) {
